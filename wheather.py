...
def load_data():
    ...
    np.random.seed(42)
    dates = pd.date_range(start='2012-01-01', end='2015-12-31')
    weather_types = ['drizzle', 'rain', 'sun', 'snow', 'fog']

    df = pd.DataFrame({
        'date': dates,
        'precipitation': np.random.uniform(0, 50, len(dates)),
        'temp_max': np.random.uniform(0, 35, len(dates)),
        'temp_min': np.random.uniform(-5, 25, len(dates)),
        'wind': np.random.uniform(0, 10, len(dates)),
        'weather': np.random.choice(weather_types, len(dates))
    })
    return df
...
def prepare_ml_data(df):
    ...
    df_cleaned['weather_encoded'] = pd.Categorical(df_cleaned['weather']).codes
...
if page == "Data Overview":
    ...
    with col2:
        st.metric(
            label="Features",
            value=len(df.columns) - 2  # Excluding date and weather
        )

    with col3:
        st.metric(
            label="Weather Types",
            value=len(df['weather'].unique())
        )
    ...
    with col2:
        ...
        for weather_type, count in weather_distribution.items():
            st.metric(
                label=weather_type,
                value=count
            )
...
# Exploratory Analysis Page
    ...
    feature_to_plot = st.selectbox("Select Feature", df.columns[1:-1])

    fig, ax = plt.subplots(figsize=(10, 6))
    sns.histplot(data=df, x=feature_to_plot, kde=True, palette='coolwarm')
    plt.title(f'Distribution of {feature_to_plot}')
    plt.xticks(rotation=45)
    st.pyplot(fig)

    # Feature vs Weather
    st.subheader("Feature vs Weather Type")
    fig, ax = plt.subplots(figsize=(12, 6))
    sns.boxplot(data=df, x='weather', y=feature_to_plot, palette='Set2')
    plt.title(f'{feature_to_plot} Distribution by Weather Type')
    plt.xticks(rotation=45)
    st.pyplot(fig)
